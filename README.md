...
Mainly there is a background queue that stores the incoming texts from post requests. There is also a background worker service is executed every 500ms to check the queue and save the texts to the text file.

There are to setting inside appsettings.json:
